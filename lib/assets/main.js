...
const { getAccount, getChainId, getChains, getToken, signMessage, getBalance, readContract, readContracts, writeContract, getBlockNumber, getGasPrice, getTransactionCount, estimateGas, getTransactionReceipt, sendTransaction, watchChainId, getTransaction, getTransactionConfirmations, getBlock, getBlockTransactionCount } = WagmiCore
...
class JSWagmiCore {
  ...
  getBlockNumber = async function (getBlockNumberParameters) {
    try {
      ...
      console.error("Error getBlockNumber:", error);
      throw error;
    }
  }
  getBlockTransactionCount = async function (getBlockTransactionCountParameters) {
    try {
      return await getBlockTransactionCount(config(), getBlockTransactionCountParameters);
    } catch (error) {
      console.error("Error getBlockTransactionCount:", error);
      throw error;
    }
  }
  ...
  sendTransaction = async function (sendTransactionParameters) {
    try {
      return await sendTransaction(config(), sendTransactionParameters);
    } catch (error) {
  ...
  getTransaction = async function (getTransactionParameters) {
    try {
      return await getTransaction(config(), getTransactionParameters);
    } catch (error) {
      console.error("Error getTransaction:", error);
      return error;
    }
  }
  getTransactionConfirmations = async function (getTransactionConfirmationsParameters) {
    try {
      return await getTransactionConfirmations(config(), getTransactionConfirmationsParameters);
    } catch (error) {
      console.error("Error getTransactionConfirmations:", error);
      return error;
    }
  }
  getBlock = async function (getBlockParameters) {
    try {
      return await getBlock(config(), getBlockParameters);
    } catch (error) {
